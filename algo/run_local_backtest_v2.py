#!/usr/bin/env python3
"""
Pure-stdlib CRT + TBS backtester  --  V2 (3 FIXES vs v1)

Fixes applied (v2 vs v1):
  1) TP is now RISK-BASED (true 1:2 RR):
        long  : tp = entry + tp_mult * (entry - sl)
        short : tp = entry - tp_mult * (sl - entry)
     v1 used `tp_mult * ATR` which under-delivered the advertised RR.
  2) min_body_pct  : 50 -> 60   (stricter CRT candle filter)
  3) use_pdl_filter: False -> True (only trade near PDH/PDL liquidity)
"""
import csv
import gzip
import os
...
class Config:
    ...
    sl_mult: float       = 1.0
    tp_mult: float       = 2.0
    min_body_pct: float  = 60.0          # FIX #2: 50 -> 60
    use_pdl_filter: bool = True          # FIX #3: enable PDH/PDL proximity
...
def main():
    ...
    if results:
        print()
        print("=" * 64)
        print("  FINAL SUMMARY V2  (true 1:2 RR, body>=60%, PDH/PDL filter ON)")
        print("=" * 64)
