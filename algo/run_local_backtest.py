#!/usr/bin/env python3
"""
Pure-stdlib CRT + TBS backtester.

Runs against pre-cloned historical-data repos (no pandas/numpy/internet
required). Outputs winrate/PF/drawdown for every pair found.

Data sources (must already be cloned next to this workspace):
  - ejtraderLabs/historical-data           (forex + XAUUSD, m15 CSV)
  - ff137/bitstamp-btcusd-minute-data      (BTC 1-min, gz CSV)
  - Vitaly007/Bitfinex-...-CryptoCompare-..(ETH 1-min, chunked TSV)
"""
import csv
import gzip
import os
...
def run_backtest(bars: List[Bar], cfg: Config, name: str = "") -> Tuple[List[Trade], float, float]:
    ...
    print(f"  [{name}] computing ATR + HTF trend ...", flush=True)
    ...
    for i in range(n):
        ...
        if (pos is None and trades_today < cfg.max_trades_day
            ...
            sig = htf_sig[i]
            buy_cond  = buy_sweep  and bull_crt and sig ==  1
            sell_cond = sell_sweep and bear_crt and sig == -1
...
def main():
    ...
    if results:
        print()
        print("=" * 64)
        print("  FINAL SUMMARY  (all pairs, RR 1:2, max 1 trade/day)")
        print("=" * 64)
